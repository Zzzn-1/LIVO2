...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1)
{
  ...
  jump_down_limit = cos(jump_down_limit / 180 * M_PI);//移除盲区内的点，并根据 point_filter_num 进行下采样
...
/*
点云数据预处理函数 avia_handler，主要用于处理来自 Livox 激光雷达的原始点云数据。代码的主要功能包括点云数据的过滤、特征提取以及分类存储
*/
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();//pl_surf、pl_corn、pl_full 是点云数据的容器，分别用于存储平面点、角点和完整的点云数据
  double t1 = omp_get_wtime();//用于记录当前时间，用于后续计算处理时间
  int plsize = msg->point_num;//是输入点云数据的点数
...
//为 pl_corn 和 pl_surf 预分配内存，以避免频繁的内存分配操作
//pl_full 被重新调整为 plsize 大小，用于存储所有有效的点云数据
  for (int i = 0; i < N_SCANS; i++)//初始化缓冲区
  {
    pl_buff[i].clear();
    pl_buff[i].reserve(plsize);//是一个数组，每个元素是一个点云容器，用于存储不同扫描线（N_SCANS）的点云数据
  }
  uint valid_num = 0;
/*
如果 feature_enabled 为真，则进入特征提取模式。
遍历点云数据，筛选出有效的点（line < N_SCANS 且 tag & 0x30 == 0x10）。
将有效的点存储到 pl_full 中，并根据点的位置变化判断是否为新的点，如果是则将其存储到对应扫描线的缓冲区 pl_buff 中
*/
...
/*
对每个扫描线的点云数据进行特征提取。如果某个扫描线的点云数据点数小于等于 5，则跳过该扫描线。计算每个点的 range（点到原点的距离平方）和 dista（与下一个点的距离平方）。
调用 give_feature 函数进行特征提取
*/
    static int count = 0;
  ...
  /*
  如果 feature_enabled 为假，则进入非特征提取模式。遍历点云数据，筛选出有效的点（line < N_SCANS）。将有效的点存储到 pl_full 中，并根据时间戳调整 curvature 值。
  每隔 point_filter_num 个点进行一次过滤，将满足条件的点存储到 pl_surf 中
  */
  else
  {
    for (uint i = 0; i < plsize; i++)
...
/*
主要用于处理来自 Intel RealSense L515 激光雷达的原始点云数据。代码的主要功能包括点云数据的过滤和存储
*/

...
public:
  ...
  bool ncc_en = false, colmap_output_en = false, setic_en = false;
  ...
  FramePtr new_frame_;
  cv::Mat img_cp, img_rgb, img_test, img_rgb_setic, img_cp_setic, img_setic;//
  cv::Mat semantic_mask_id_;
  bool semantic_mask_valid_ = false;
  double semantic_mask_time_ = -1.0;
  double semantic_sync_threshold_ = 0.05;

  enum SemanticClassId : uint8_t
  {
    SEM_BACKGROUND = 0,
    SEM_PERSON = 1,
    SEM_CAR = 2,
  ...
  void setSemanticMask(const cv::Mat &semantic_mask, double semantic_time, double rgb_time);

  void processFrame_setic(cv::Mat &img_setic, vector<pointWithVar> &pg_setic, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map_setic);
  // void processFrame_setic(cv::Mat &img_setic, std::vector<pointWithVar> &points,const std::unordered_map<VOXEL_LOCATION, VoxelOctoTree*> &voxel_map);
  ...
  // void processFrame_setic(cv::Mat &img_setic, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map);
  ...
  bool isDynamicMaskPixel(const V2D &pc) const;

...
typedef struct pointWithVar {
  ...
  Eigen::Matrix3d var_nostate; // the var removed the state covariance
  Eigen::Matrix3d body_var;
  Eigen::Matrix3d var;
  Eigen::Matrix3d point_crossmat;
  Eigen::Vector3d normal;
  float variance;              // 方差
  int semantic_label;          // 语义标签
  ...
  // 新增的构造函数
  pointWithVar(const Eigen::Vector3d &point, const Eigen::Matrix3d &covariance, int label)
      : point_w(point), var(covariance), semantic_label(label)
  {
      point_b = Eigen::Vector3d::Zero();
      point_i = Eigen::Vector3d::Zero();
      var_nostate = Eigen::Matrix3d::Zero();
